...
import type { BubbleBobbleScoreRecord } from "@/lib/types";

const SCORES_COLLECTION = "bubbleBobbleScores";

export function listenBubbleBobbleScores(
  callback: (scores: BubbleBobbleScoreRecord[]) => void,
  onError?: (error: FirestoreError) => void,
) {
  const q = query(collection(db, SCORES_COLLECTION), orderBy("bestScore", "desc"));
  return onSnapshot(
    q,
    (snap) => callback(snap.docs.map((d) => d.data() as BubbleBobbleScoreRecord)),
    (error) => {
      ...
      console.error("bubbleBobbleScores listener failed", error);
      onError?.(error);
    },
  );
}
 ...
 * Bubble Bobble" push notification. bestScore only ever increases for a
 ...
 */
export async function submitBubbleBobbleScore(
  personId: string,
  score: number,
  ...
  const allScores = topSnap.docs.map((d) => d.data() as BubbleBobbleScoreRecord);
