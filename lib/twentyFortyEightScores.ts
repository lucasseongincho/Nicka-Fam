...
import type { TwentyFortyEightScoreRecord } from "@/lib/types";

const SCORES_COLLECTION = "twentyFortyEightScores";

export function listenTwentyFortyEightScores(
  callback: (scores: TwentyFortyEightScoreRecord[]) => void,
  onError?: (error: FirestoreError) => void,
) {
  const q = query(collection(db, SCORES_COLLECTION), orderBy("bestScore", "desc"));
  return onSnapshot(
    q,
    (snap) => callback(snap.docs.map((d) => d.data() as TwentyFortyEightScoreRecord)),
    (error) => {
      ...
      console.error("twentyFortyEightScores listener failed", error);
      onError?.(error);
    },
  );
}
 ...
 * 2048" push notification. bestScore only ever increases for a given
 * person, so the only rank changes possible are the submitter moving up
 * past people whose score falls in (oldBest, newScore].
 */
export async function submitTwentyFortyEightScore(
  personId: string,
  score: number,
  ...
  const allScores = topSnap.docs.map((d) => d.data() as TwentyFortyEightScoreRecord);
