...
import { listenBubbleShooterScores } from "@/lib/bubbleShooterScores";
import type { BubbleShooterScoreRecord } from "@/lib/types";

export function BubbleShooterLeaderboard() {
  const { people, activePersonId } = usePeople();
  const [scores, setScores] = useState<BubbleShooterScoreRecord[] | null>(null);
  const [failed, setFailed] = useState(false);

  useEffect(() => listenBubbleShooterScores(setScores, () => setFailed(true)), []);
