...
import { listenTwentyFortyEightScores } from "@/lib/twentyFortyEightScores";
import type { TwentyFortyEightScoreRecord } from "@/lib/types";

export function TwentyFortyEightLeaderboard() {
  const { people, activePersonId } = usePeople();
  const [scores, setScores] = useState<TwentyFortyEightScoreRecord[] | null>(null);
  const [failed, setFailed] = useState(false);

  useEffect(() => listenTwentyFortyEightScores(setScores, () => setFailed(true)), []);
