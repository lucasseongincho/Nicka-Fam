...
import { listenBubbleBobbleScores } from "@/lib/bubbleBobbleScores";
import type { BubbleBobbleScoreRecord } from "@/lib/types";

export function BubbleBobbleLeaderboard() {
  const { people, activePersonId } = usePeople();
  const [scores, setScores] = useState<BubbleBobbleScoreRecord[] | null>(null);
  const [failed, setFailed] = useState(false);

  useEffect(() => listenBubbleBobbleScores(setScores, () => setFailed(true)), []);
