...
import type { BubbleShooterScoreRecord } from "@/lib/types";

const SCORES_COLLECTION = "bubbleShooterScores";

export function listenBubbleShooterScores(
  callback: (scores: BubbleShooterScoreRecord[]) => void,
  onError?: (error: FirestoreError) => void,
) {
  const q = query(collection(db, SCORES_COLLECTION), orderBy("bestScore", "desc"));
  return onSnapshot(
    q,
    (snap) => callback(snap.docs.map((d) => d.data() as BubbleShooterScoreRecord)),
    (error) => {
      ...
      console.error("bubbleShooterScores listener failed", error);
      onError?.(error);
    },
  );
}
 ...
 * Bubble Shooter" push notification. bestScore only ever increases for a
 ...
 */
export async function submitBubbleShooterScore(
  personId: string,
  score: number,
  ...
  const allScores = topSnap.docs.map((d) => d.data() as BubbleShooterScoreRecord);
