...
import { listenCandyMatchScores } from "@/lib/candyMatchScores";
import type { CandyMatchScoreRecord } from "@/lib/types";

export function CandyMatchLeaderboard() {
  const { people, activePersonId } = usePeople();
  const [scores, setScores] = useState<CandyMatchScoreRecord[] | null>(null);
  const [failed, setFailed] = useState(false);

  useEffect(() => listenCandyMatchScores(setScores, () => setFailed(true)), []);
